...
@internal
 */
export function getScreenSpaceLaserIntersections(laserPosition: ReadonlyVec3, onlyOnOutlines: boolean, context: RenderContext, width: number, height: number, convert: ScreenSpaceConversions, xDir?: ReadonlyVec3, yDir?: ReadonlyVec3, zDir?: ReadonlyVec3, autoAlign?: boolean): Intersection | undefined {
        ...
        if (autoAlign && xDir && yDir) {
            ...
            const getRotatedLaser = (point: ReadonlyVec3,
                ...
                if (line1 && line2) {
                    ...
                    if (l1 < l2) {
                        len = l1;
                        const newDir1 = vec3.scale(vec3.create(), line1, 1 / l1);
                        const newDir2 = vec3.cross(vec3.create(), newDir1, normal);
                        vec3.copy(testPoint, testPoint1.point[0]);
                        if (horizontal) {
                            if (!negate) {
                                negate = true;
                            }
                        ...
                        len = l2;
                        const newDir1 = vec3.scale(vec3.create(), line2, 1 / l2);
                        const newDir2 = vec3.cross(vec3.create(), newDir1, normal);
                        vec3.copy(testPoint, testPoint2.point[0]);
